...
// SearchDocuments runs a Portuguese FTS query over document_chunks and returns
// up to limit results ranked by ts_rank. The excerpt is generated by
// ts_headline with the default configuration. Returns an empty slice (never
// nil) when no chunks match the query.
func (r *PostgresRepository) SearchDocuments(query string, limit int) ([]docsearch.SearchResult, error) {
	ctx := context.Background()
	...
	`, query, limit)
	if err != nil {
		return nil, fmt.Errorf("postgres: document search: %w", err)
	}
	defer rows.Close()
