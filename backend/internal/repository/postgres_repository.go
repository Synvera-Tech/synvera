...
// three-stage cascade:
//
//  1. AND search: plainto_tsquery over all stopword-normalized terms.
//  2. Per-term fallback: if AND returns nothing, try each meaningful term
//     individually from last to first (the last term in a PT question is
//     usually the main subject, e.g. "auxiliares" in "Qual é a regra para auxiliares?").
//  3. ILIKE: last resort substring match on the most specific term.
//
// Returns an empty slice (never nil) when no chunks match.
func (r *PostgresRepository) SearchDocuments(query string, limit, offset int, docType string) ([]docsearch.SearchResult, error) {
	normalized, terms := docsearch.NormalizeQuery(query)

	// Stage 1: AND search with all normalized terms.
	// Offset is applied here to support "load more" pagination on the primary query.
	results, err := r.searchDocumentsFTS(normalized, limit, offset, docType)
	if err != nil {
		return nil, err
	}
	if len(results) > 0 {
	...
	// Stage 2: per-term FTS, last-to-first (fallback — offset always 0).
	// In Portuguese questions the main subject is usually the last word,
	// e.g. "auxiliares" in "Qual é a regra para auxiliares?".
	for i := len(terms) - 1; i >= 0; i-- {
		termResults, err := r.searchDocumentsFTS(terms[i], limit, 0, docType)
		if err != nil {
			continue
		}
	...
	// Stage 3: ILIKE on the last (most specific) term — last resort.
	if len(terms) > 0 {
		results, err = r.searchDocumentsILIKE(terms[len(terms)-1], limit, docType)
		if err != nil {
			return nil, err
		}
	}
...
// searchDocumentsFTS executes a plainto_tsquery AND search.
// docType filters by document_type column; pass "" for all documents.
// offset supports "load more" pagination.
func (r *PostgresRepository) searchDocumentsFTS(query string, limit, offset int, docType string) ([]docsearch.SearchResult, error) {
	ctx := context.Background()
	...
	rows, err := r.pool.Query(ctx, `
		SELECT
			...
			ts_headline(
				...
				'MaxWords=55, MinWords=25, StartSel=«, StopSel=»'
			) AS excerpt,
			ts_rank(dc.search_vector, plainto_tsquery('portuguese', $1)) AS score
		FROM document_chunks dc
		JOIN documents d ON d.id = dc.document_id
		WHERE dc.search_vector @@ plainto_tsquery('portuguese', $1)
		  AND ($3::text = '' OR d.document_type = $3)
		ORDER BY score DESC
		LIMIT $2
		OFFSET $4
	`, query, limit, docType, offset)
	if err != nil {
		return nil, fmt.Errorf("postgres: document search FTS: %w", err)
	}
	defer rows.Close()
...
// searchDocumentsILIKE is a last-resort substring search used when FTS
// returns nothing. It does not highlight matches.
// docType filters by document_type column; pass "" for all documents.
func (r *PostgresRepository) searchDocumentsILIKE(term string, limit int, docType string) ([]docsearch.SearchResult, error) {
	ctx := context.Background()

	// Sanitise: strip SQL wildcard metacharacters from the user term.
	safe := strings.ReplaceAll(strings.ReplaceAll(term, "%", ""), "_", "")
	if safe == "" {
	...
	rows, err := r.pool.Query(ctx, `
		...
		WHERE (dc.chunk_text    ILIKE $1
		    OR dc.section_title ILIKE $1)
		  AND ($2::text = '' OR d.document_type = $2)
		LIMIT $3
	`, pattern, docType, limit)
	if err != nil {
		return nil, fmt.Errorf("postgres: document search ILIKE: %w", err)
	}
	defer rows.Close()
...
func scanDocumentRows(rows pgx.Rows) ([]docsearch.SearchResult, error) {
	results := make([]docsearch.SearchResult, 0)
	for rows.Next() {
		var r docsearch.SearchResult
		if err := rows.Scan(&r.Document, &r.Version, &r.Page, &r.Section, &r.Excerpt, &r.Score); err != nil {
			return nil, fmt.Errorf("postgres: document search scan: %w", err)
		}
